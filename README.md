...
宠物的名字会显示在宠物窗口**最上面那一行**，默认是「终端宠物 · 小喵」，
其中「小喵」就是名字。改名字有两种方法，任选其一。
...
执行后会提示「已把宠物名字改为…」，重新打开宠物窗口（`pet`）就能看到。
名字支持中文、英文或 emoji，例如：
...
宠物的配置保存在一个叫 `config.json` 的文件里，它的完整路径是：

```
C:\Users\<你的用户名>\.terminal-pet\config.json
```

也就是 `%USERPROFILE%\.terminal-pet\config.json`。`%USERPROFILE%` 代表
`C:\Users\<你的用户名>`，例如你的账户就是 `C:\Users\lenovo`。

- **文件还不存在？** 运行一次 `pet` 或 `pet-run`，它会自动生成；也可以手动新建。
- 文件内容是一个 JSON，默认长这样：
...
- 把 `"小喵"` 换成你想要的名字——**只改引号里面的字，保留英文双引号**，例如：
...
- 用 UTF-8 编码保存文件，然后重新打开 `pet` 窗口即可生效。

> 注意：JSON 必须用**英文双引号** `"`（不是中文引号），花括号 `{ }` 要成对。
> 如果文件写坏了，宠物会自动退回默认名字「小喵」，不会报错。
